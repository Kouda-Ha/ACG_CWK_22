...
{
    for (TexturedLit* obj : subObjects)
        obj->Draw(trans, light);
}
...
void MeshObject::Load(const std::string& fileName)
{
    ...
    for (int i = 0; i < numMeshes; i++)
    {
        ...
        for (int i : indices)
        {
            positions.push_back(vPos[i]);
            normals.push_back(vNorm[i]);
            tangents.push_back(vTangent[i]);
            uvs.push_back(vUV[i]);

        }
        BasicGeometryHelper geom(positions, normals, tangents, uvs);
